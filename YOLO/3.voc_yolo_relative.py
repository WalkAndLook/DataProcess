...
Using: 这里采用的都是相对路径，运行的时候需要保持文件名，格式，位置一致，只需要修改classes
File_before:
    dataset
        Annotations  # 存放标签xml的位置
        images       # 存放图片jpg的位置
        ImageSets    # 之前划分数据集的文件名
    3.voc_yolo_relative.py
File_after:
    dataset
        Annotations  # 存放标签xml的位置
        images       # 存放图片jpg的位置
...
def convert_annotation(image_id):
    in_file = open(r'./dataset/Annotations/%s.xml' % (image_id), encoding='UTF-8')
    out_file = open(r'./dataset/labels/%s.txt' % (image_id), 'w')
    ...
    if size == None:
        ...
        img = Image.open('./dataset/images/' + image_id + '.jpg')
        w, h = img.size  # 大小/尺寸
...
for image_set in sets:
    if not os.path.exists(r'./dataset/labels'):
        os.makedirs(r'./dataset/labels')
    image_ids = open(r'./dataset/ImageSets/%s.txt' % (image_set)).read().strip().split()
    list_file = open(r'./dataset/%s.txt' % (image_set), 'w')
    # 这行路径不需更改，这是相对路径
